...
export default {
  ...
  actions: {
    async aSearchDocs({commit}, payload){
      try {
        const all = await axios.get(`/tasks/aparat/search?description=${payload}`)
        console.log(all)
    ...
    async aPageSelect({commit}, payload){
      console.log(payload)
      try {
        const all = await axios.post(`files/docs/apparat?page=${payload.page}&rowsPerPage=${payload.perPage}`, payload.filters)
        const arrPage = {
    ...
    async aAllFilters({commit}){
      try {
        ...
        const seniors = [], regions = [], organs = [], departments = [], statuses = []
