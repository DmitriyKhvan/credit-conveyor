    ...
    <div class="row q-pa-md">
      <div class="col-8 q-pr-md">
        ...
        <div class="comments scroll q-pb-md" style="max-height: 45vh">
          <div class="comment q-pt-md" v-for="(c, i) in 3" :key="i">
            ...
            <div class="textComment">
              ...
              <div class="content">короткое содержания комментария</div>
              <div class="actions">
        ...
        <div class="addComment q-py-md">
          ...
          <div class="addText">
            <q-input v-model="text" outlined type="textarea" label="Outlined" dense />
            <q-btn color="white q-mt-sm" text-color="black" :label="$t('actions.send')" />
          </div>
        </div>
      </div>
      ...
      <div class="col-4 q-pa-md rightBlock">
        <div class="rightActions">
          <div class="buttonLeft">
            <q-select dense filled v-model="model" :options="options" :label="$t('common.status')" />
          </div>
          <div class="q-pl-md">
            <q-btn color="primary" text-color="white" :label="$t('actions.send')" />
          </div>
        </div>
        <div class="q-py-md">
          <q-select
            ...
            v-model="model"
            :options="options"
            :label="$t('tables.work.tasks.dialog_task.choose_executor')"
          />
        </div>
        <div class="usersRight q-pb-md">
          <q-chip color="red" text-color="white">Хакимов Ф.Е.</q-chip>
          <div class="userLine">Мирсаев У.А.</div>
          <div class="userLine">Хамдамов А.А.</div>
          <div class="userLine">Баратов С</div>
        </div>
        <q-separator />
        <div class="general">{{$t('common.from')}}: Мамхмудов К.Р</div>
        <q-separator />
        ...
        <q-separator />
        <div class="date">
          {{$t('common.created')}}: 20.01.2019
          <br />
          {{$t('tables.work.tasks.dialog_task.update')}}: 1 день назад
        </div>
      </div>
    </div>
