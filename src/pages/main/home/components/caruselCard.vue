<template>
  <div>
    ...
    >
      ...
      >
        <q-card class="userBirthday">
          <div class="userPad">
            <div
              v-if="user.get_department_tree"
              class="blue q-pb-sm text-weight-bold"
              v-html="filial(user.get_department_tree)"
            ></div>
            ...
            <div class="row">
              ...
              <div class="col-8 q-px-sm">
                <div
                  class="blue text-body2"
                  v-html="name(user.LAST_NAME, user.FIRST_NAME, user.MIDDLE_NAME )"
                ></div>
            ...
            <div class="row q-pt-md justify-between">
              <div
                class="col-4 greyColor vertical-middle"
              >{{$t('main.caruselCard.birthday')}} {{dateFormat(user.BIRTH_DATE)}}</div>
              <div class="col-8 redColor text-right">
                <q-icon name="card_giftcard" style="font-size: 1.6em" />
              </div>
            </div>
          </div>
        </q-card>
    ...
    <div class="text-center q-pa-sm">
      ...
      {{$t('main.caruselCard.employees')}} {{$t('main.caruselCard.celebrates')}}
      <span
        v-if="datetime === 'today'"
      >{{$t('main.caruselCard.today')}}</span>
      <span v-else-if="datetime === 'tomorrow'">{{$t('main.caruselCard.tomorrow')}}</span>
      <span v-else>{{$t('main.caruselCard.aftertomorrow')}}</span>
      {{$t('main.caruselCard.birthday')}}
    </div>
  </div>
</template>
