    ...
    <q-card class="primary">
      ...
      <q-card-section>
        <q-table
          ...
          :filter="filter"
          :loading="loading"
          @update:selected="clickEvent(selectedRows)"
        >
...
export default {
  ...
  methods: {
    ...
    viewRow() {
      console.log(this.selectedRows[0][this.rowId]);
      this.$store.commit("setTopicId", this.selectedRows[0][this.rowId]);
    },
    async refreshTable() {
      this.loading = true;
      this.clearTableData();
    ...
    clickEvent(details) {
      if (details[0]) {
        ...
        this.$store.commit("setTopicId", details[0].id);
      }
    }
  }
};
